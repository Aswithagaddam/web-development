...
// Derived class from Item, specifically for items added to the cart
class CartItem : public Item {
public:
    int quantity;

    // Constructor to initialize a CartItem and base Item properties
...
// Cart to store items the user adds
vector<CartItem> cart;
unordered_set<string> purchasedCategories;  // Tracks categories where purchases were made

// Template function for adding an item to the cart from any section
template <typename T>
...
// Abstract base class for item suggestions
class Suggestion {
public:
    virtual void suggestBasedOnPurchase() const = 0;  // Pure virtual function
};

// Derived classes for specific category suggestions
...
// New derived classes for other categories
...
// Function to suggest items based on categories in purchase history
void suggestItems() {
    if (!purchasedCategories.empty()) {
        cout << "\n--- Item Suggestions Based on Purchase History ---\n";
        
        // Iterate over categories where purchases were made
        for (const auto &category : purchasedCategories) {
            ...
            if (suggestion) {
                suggestion->suggestBasedOnPurchase();  // Calls the overridden function
                delete suggestion;
            }
        }
    }
...
// Function to search for items by name in all categories
...
int main() {
    ...
    displayBill();    // Display final bill
    suggestItems();   // Provide suggestions based on purchase history
